...
function showLoading(){
	try {
		 $.blockUI({
			 message: "<img src='/resources/images/bg/loading_bar.gif' />", 
			 css: {
...
var default_image="/resources/images/bg/default_img.jpg";   //기본이미지
var default_image2="/resources/images/bg/default_img02.jpg";   //세로가 가로보다 긴 이미지
	...
	*/
	...
	try{
		if(nullToBlank(device.uuid) != ""){
			...
			cfn_ajax({
				url: "/setting/insertMarketingReceiveYn.do",
				type: "POST",
				data: formData,
				async: false,
				cache: false,
...
var fn_goToList = function() {
	document.location.href="/event/event_list.do";
}
...
var fn_hotelMove = function(){
	location.href = "/domestic/dmt_detail_domestic.do?url=http://m.cp.hotelpass.net/search.asp?cp_agent=jtbmo&ctgname=호텔";
...
function gfn_loading_bar_draw(_flag) {
	...
	if($('#test_loading_bar_img').length < 1){
		$('#test_loading_bar').append('<img id="test_loading_bar_img" src="/resources/images/img/loading.gif" style="width:296px;"></img>');
	}
...
var fn_moveFreePage = function(type){
	if(nullToBlank(type) != ""){
		location.href = "/free/main.do?tabType="+ type;
	}else{
		location.href = "/free/main.do";
	}
}
...
function fn_keywordSearch(){
    ...
    if(nullToBlank($("#autoComplete").val()) == "" && mentoYn == "N"){
    	...
    	if(mentoYn == "Y"){
    		...
    		location.href = "/search/search.do?searchKeyword="+encodeURI(encodeURIComponent(catg)) + "&sechType=TM";				
    	}else{
    		
    		if(nullToBlank($("#autoComplete").val()) != ""){
    			location.href = "/search/search.do?searchKeyword="+$("#autoComplete").val();				
    		}else{
    			setAlert("검색어를 입력하세요.");	
    		}
...
function fn_lpoinIntro(){
	location.href = "/corperate/corperate_lpoint.do";
}