...
<%
	...
	serverDay = serverDay.substring(0, 4) + "." + serverDay.substring(4, 6) + "." + serverDay.substring(6, 8);			
%>
<script>
	var mtOtherCnt = 3;
	var availTy = 'RT';
	...
	function fn_addTravelPlan() {
		...
		if(mtOtherCnt < 5) {
			...
			html = html + " 	<input type='hidden' id='mtcnt"+mtOtherCnt+"' name='mtcnt' value='"+mtOtherCnt+"' />";	
			html = html + "	</div>";
			...
			html = html + "		<li><button type='button' class='layerOpener' data-url='/pop/pop_calendar.do?type=AIR&availTy=MT&travelMtCnt="+ mtOtherCnt + "'  data-ctn='full' data-role='none' style='cursor='hand';' data-ok='yes'><span class='noSelect' name='depDt' id='dep_dt"+mtOtherCnt+"'>출발일선택</span></button></li>";
			html = html + "	</ul>";
			html = html + "</div>";
...
<!-- 항공파트 -->
<div class="chkpart airpart" ft-att="srchKind" id="air_search_div">
	<!-- 항공텝 -->
	<p class="pop_avtitle">항공</p>
	...
	<ul class="tablist_com free_airtab" data-ctn="airbox">
		...
		<li><button type="button" data-role="none" id="chkH" name="availTy" onclick="fn_chnAvailTy('MT');" value='MT'>다구간</button></li>
		<li><button type="button" data-role="none" id="chkM" name="availTy" onclick="fn_chnAvailTy('OO');" value='OO'>해외출발</button></li>
	</ul>
	<!-- 왕복 도시, 날짜, 인원 선택 -->
	...
	<div class="tabctn_com airbox airbox3" ft-att="MTGroup" ar-att="availgroup">
		...
		<div class="many_travelbox">
			...
			<div class="stable_guide oenway">
				<button type="button" class="layerOpener" data-url="/pop/pop_city_select1.do?availTy=MT&travelMtCnt=1&type=D" data-width="300" data-height="400" data-ctn="full" data-role="none" style="cursor='hand';">
					<p class="st_text">
						<span name="depCd" id="srch_dep_ct">SEL</span>
						<input type='hidden' id='srch_dep_naCd' name='srch_dep_naCd' value='KR' />
					</p>
