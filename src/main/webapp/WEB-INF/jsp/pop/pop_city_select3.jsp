...
<script type="text/javascript">	
	...
	var fn_init = function() {
		...
		html = html + "							<input type='text' data-role='none' placeholder='도시, 호텔, 인근도시, 공항 등' id='cityPopInput' title='검색어를 입력하세요.' onkeyup='fn_keyupCitySearch();'/>";
		html = html + "							<button type='button' data-role='none' onclick='fn_keyupCitySearch()'>검색</button>";
		html = html + "						</fieldset>";
		html = html + "					</div>";
		// <!-- 검색 결과 --> //
		...
		for (var c_idx = 0; c_idx < baseCityList.length; c_idx++) {
			...
			html = html + "					<tr>";
			html = html + "						<th scope='row' rowspan='" + row_cnt + "'>" + cityGroup.naNm + "</th>";

			//항공 도시 검색
			for (var l_idx = 0; l_idx < cityGroup.cityList.length; l_idx++) {
				var city_list = cityGroup.cityList[l_idx];
				//var funcNm = "selectCityList('" + city_list.ctCd + "','" + city_list.ctCd + "("+ city_list.naNm +")','"+ city_list.naCd +"')";
				var funcNm = "selectCityList('" + city_list.rntCd + "','" + city_list.ctNm + "','"+ city_list.naCd +"')";
				html = html + '				<td city-data="' + city_list.rntCd + '" onClick="' + funcNm + '">' + city_list.ctNm + '</td>';
	...
	// 도시검색 팝업 선택 
	function selectCityList(cityCd, cityNm, countryCd){
		...
		if(type == 'A') { // 도착 : A 도착지 선택 
			availGroup.find("#return_place_cd").val(cityCd);
			availGroup.find("#return_place_oagCd").val('');
			availGroup.find("#return_place_countryCd").val(countryCd);
			
			availGroup.find("#arr_place").val(cityNm);
		} else { // 출발 : D 출발지 선택 
			availGroup.find("#take_place_cd").val(cityCd);
			availGroup.find("#take_place_oagCd").val('');
			availGroup.find("#take_place_countryCd").val(countryCd);
			
			availGroup.find("#dep_place").val(cityNm);
	...
	function fn_searchKeyupCity(_val, _obj){
		var nowVal = $("#cityPopInput").val();
		if(nowVal == null) nowVal = "";
		
		if(nowVal == _val){	// 입력 .5초후 값과 비교
			gfn_callAjax ('/fit/rnt/listCityL.do', {'searchVal' : nowVal.toUpperCase()}, 'fn_search_rc_city', true);
		} else {
			clearTimeout();
		}
	}
	
	function fn_search_rc_city(_data){
		...
		if(_data.cityList.length > 0){
			...
			for(var r_idx=0; r_idx<reData.length; r_idx++) {
				var viewNm = reData[r_idx].cityNm + ' (' + reData[r_idx].country + ')' + ' - ' + reData[r_idx].nationKrNm; 
				//var funcNm = "javascript:selectCityList('" +reData[r_idx].city+ "','" + reData[r_idx].city +"("+ reData[r_idx].nationKrNm +")" + "', '"+ reData[r_idx].country +"')";
				var funcNm = "javascript:selectCityList('" +reData[r_idx].city+ "','" + reData[r_idx].cityNm + "', '"+ reData[r_idx].country +"')";
