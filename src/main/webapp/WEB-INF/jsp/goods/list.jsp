...
<script type="text/javascript" src="//static.criteo.net/js/ld/ld.js" async="true" ver="111"></script>
					...
					if(list != null) {
						...
						for(var i=0; i < list.length; i++) {
							...
							if(list[i]["save_file_nm"] != null) {
								...
								if(list[i]["goods_cd"] == 'KJK2366') {
									html = html + "			<span class='img2' style='background:url(/m_file" + "/upload/common/goods/" + list[i]["goods_cd"].substring(0,2) + "/"  + list[i]["goods_cd"] + "/" + fn_chgImage(list[i]["save_file_nm"], 'L') + ")'>";
								}else{
									html = html + "			<span class='img' style='background:url(/m_file" + "/upload/common/goods/" + list[i]["goods_cd"].substring(0,2) + "/"  + list[i]["goods_cd"] + "/" + fn_chgImage(list[i]["save_file_nm"], 'L') + ")'>";
								}
							...
							if(grade == "그룹직원" && parseInt(list[i]["emp_rate"]) > 0){	
								...
								html = html + "			<span class='holedate' style='top:50px;'>여행기간1 : "+list[i]["goods_nights"] + "</span>";
								html = html + "			<span class='startday' style='top:68px;'>출발요일1 : "+ list[i]["et_days"].replace(/\|/g, ',') +"</span>";
								...
								html = html + "			<span class='holedate'>여행기간2 : "+list[i]["goods_nights"] + "</span>";
								html = html + "			<span class='startday'>출발요일2 : "+ list[i]["et_days"].replace(/\|/g, ',') +"</span>";
					...
					if(list2 != null) {
						...
						for (i = 0; i < list2.length; i++) {
								...
								if(grade == "그룹직원" && parseInt(list2[i]["emp_rate"]) > 0){	
									...
									html = html + "			<span class='holedate' style='top:50px;'>여행기간 3: "+list2[i]["goods_nights"] + "</span>";
									html = html + "			<span class='startday' style='top:68px;'>출발요일3 : "+ list2[i]["et_days"].replace(/\|/g, ',') +"</span>";
									...
									html = html + "			<span class='holedate'>여행기간 4: "+list2[i]["goods_nights"] + "</span>";
									html = html + "			<span class='startday'>출발요일4 : "+ list2[i]["et_days"].replace(/\|/g, ',') +"</span>";
		...
		<div  class="list prolist" id="content">
			...
			<div data-role="navbar" data-iconpos="right" class="tabsnav scTab" style="z-index:20;width:100%">
				...
				<ul class="selec">
					...
					<li><a href="javascript:popGoodsOrderBy()"  class="ui-btn-active ui-nodisc-icon ui-alt-icon shoparea" ><span id="sp_orderby" name="sp_orderby">추천상품순<span id="sp_orderby" name="span_orderby"></a></li>
					<li><a href='javascript:void(0);' class="chgview" onclick="toggle_visibility('tab2');"></a></li>
				</ul>
			</div>
