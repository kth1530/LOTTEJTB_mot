...
public class CommResourceCloseHelper {
	private CommResourceCloseHelper(){
	}
		...
		for (Closeable resource : resources) {
			if (resource != null) {
				try {
					...
					CommBasicLogger.ignore("Occurred Exception to close resource is ingored!!");
				}
			}
		}
	}
	...
	public static void closeSocketObjects(Socket socket, ServerSocket server) {
		if (socket != null) {
			try {
				...
				CommBasicLogger.ignore("Occurred Exception to shutdown ouput is ignored!!");
			}
			
			try {
				socket.close();
			} catch (Exception ignore) {
				CommBasicLogger.ignore("Occurred Exception to close resource is ignored!!");
			}
		}
